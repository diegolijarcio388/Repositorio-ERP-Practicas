...
const getTaskAssignmentKey = (workPackageTempId: string, task: Pick<TaskDraft, "tempId" | "name">): string =>
  `${workPackageTempId}::${task.tempId}::${normalizeTaskName(task.name)}`;
...
export function AdminProjectsWorkspace({ session }: Props) {
  ...
  const [taskAssignmentsDraft, setTaskAssignmentsDraft] = useState<Record<string, string[]>>({});
  ...
  const [entries, setEntries] = useState<ProjectTimeEntryRecord[]>([]);

  const load = async () => {
    setLoading(true);
    try {
      const [ps, expenses, usersRes, depsRes] = await Promise.all([
      ...
      const users: DirectoryUser[] = usersRes.items ?? [];
      setAllUsers(users);
      setManagers(users.filter((u) => u.role === "coordinator" || u.role === "admin"));
  ...
  const loadEntries = async () => {
    const es = await projectsApi.listProjectTimeEntries({
  ...
  const managerDepartmentId = !editProjectId
    ? allUsers.find((u) => u.id === form.managerId)?.departmentId ?? ""
    : "";
  const managerDepartmentName =
    departments.find((d) => d.id === managerDepartmentId)?.name ?? "—";
  const candidateWorkers = managerDepartmentId
    ? allUsers.filter((u) => u.departmentId === managerDepartmentId && u.role === "worker")
    : [];
  const selectedProjectHourBagWorkerOptions = useMemo(() => {
    if (!selectedProject) return [];
    const selectedManagerDepartmentId = allUsers.find((u) => u.id === selectedProject.managerId)?.departmentId;
  ...
  useEffect(() => { load(); }, []);
  useEffect(() => { if (activeTab === "hours") loadEntries(); }, [activeTab, filterProjectId, filterTaskName, filterDate]);
  useEffect(() => {
    if (editProjectId) return;
    if (!managerDepartmentId) {
      setAssignedUserIdsDraft([]);
      setTaskAssignmentsDraft({});
      return;
    }
    const validUserIds = new Set(candidateWorkers.map((worker) => worker.id));
    setAssignedUserIdsDraft((current) => current.filter((id) => validUserIds.has(id)));
    setTaskAssignmentsDraft((current) => {
      const next: Record<string, string[]> = {};
      for (const [taskId, users] of Object.entries(current)) {
        next[taskId] = users.filter((id) => validUserIds.has(id));
      }
      return next;
    });
  }, [editProjectId, managerDepartmentId, allUsers, form.managerId]);
  ...
  }, [editProjectId, form.hourTrackingMode]);
  useEffect(() => {
  ...
  const toggleAssignedUser = (userId: string) => {
    ...
    setTaskAssignmentsDraft((current) => {
      const next = { ...current };
      for (const [taskTempId, assigned] of Object.entries(next)) {
        if (assigned.includes(userId)) {
          next[taskTempId] = assigned.filter((id) => id !== userId);
        }
      }
      return next;
    });
  ...
  const toggleTaskAssignment = (assignmentKey: string, userId: string) => {
    setTaskAssignmentsDraft((current) => {
      const assigned = current[assignmentKey] ?? [];
      const nextAssigned = assigned.includes(userId)
        ? assigned.filter((id) => id !== userId)
        : [...assigned, userId];
      return { ...current, [assignmentKey]: nextAssigned };
    });
  };
  ...
  const removeWorkPackageDraft = (workPackageTempId: string) => {
    setWorkPackagesDraft((current) => {
      const removed = current.find((wp) => wp.tempId === workPackageTempId);
      if (removed) {
        setTaskAssignmentsDraft((prev) => {
          const next = { ...prev };
          Object.keys(next).forEach((key) => {
            if (key.startsWith(`${removed.tempId}::`)) delete next[key];
          });
          return next;
        });
      }
      return current.filter((wp) => wp.tempId !== workPackageTempId);
    });
  };
  ...
  const removeTaskDraft = (workPackageTempId: string, taskTempId: string) => {
    ...
    setTaskAssignmentsDraft((current) => {
      const next = { ...current };
      Object.keys(next).forEach((key) => {
        if (key.startsWith(`${workPackageTempId}::${taskTempId}::`)) delete next[key];
      });
      return next;
    });
  };
  ...
  const toggleBuildingBlockSelection = (bbCode: string, checked: boolean) => {
    setWorkPackagesDraft((current) => {
      ...
      setTaskAssignmentsDraft((prev) => {
        const next = { ...prev };
        Object.keys(next).forEach((key) => {
          if (key.startsWith(`${exists.tempId}::`)) delete next[key];
        });
        return next;
      });
    ...
    setWorkPackagesDraft((current) =>
      current.map((wp) => {
        ...
        if (checked) {
          ...
          return {
            ...wp,
            tasks: [...wp.tasks, createBuildingBlockTaskDraft(wp.tempId, taskName)],
          };
        }
        if (!existingTask) return wp;
        setTaskAssignmentsDraft((previous) => {
          const next = { ...previous };
          Object.keys(next).forEach((key) => {
            if (key.startsWith(`${wp.tempId}::${existingTask.tempId}::`)) delete next[key];
          });
          return next;
        });
        return {
          ...wp,
          tasks: wp.tasks.filter((task) => task.tempId !== existingTask.tempId),
        };
      }),
    );
  };
  ...
  const handleSaveProject = async () => {
    ...
    try {
      if (editProjectId) {
        ...
        if (!form.managerId) {
          setToast({ tone: "error", message: "Selecciona un responsable del proyecto." });
          return;
        }
          ...
          for (const wp of workPackagesDraft) {
            ...
            for (const task of wp.tasks) {
              ...
              if (task.startDate && task.endDate && task.startDate > task.endDate) {
                setToast({
                  tone: "error",
                  message: `Rango de fechas inválido en la tarea "${task.name}" (${wp.name}).`,
                });
                return;
              }
            }
        ...
        if (
          ...
          if (form.hourTrackingMode === "BUILDING_BLOCK") {
            ...
            workPackagesDraft.forEach((wpDraft) => {
              ...
              wpDraft.tasks.forEach((taskDraft) => {
                ...
                const assignedUsers = taskAssignmentsDraft[getTaskAssignmentKey(wpDraft.tempId, taskDraft)] ?? [];
                assignedUsers.forEach((assignedUserId) => {
                  assignmentCalls.push(
                    projectsApi.assignUserToTask(created.id, createdTask.id, assignedUserId),
                  );
                });
              });
            });
          ...
          if (assignmentCalls.length > 0) {
            await Promise.all(assignmentCalls);
          }
        }
  ...
  const handleRejectExpense = async (id: string) => {
    if (!rejectReason.trim()) { setToast({ tone: "error", message: "Indica el motivo." }); return; }
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div>
          <h2 className="text-lg font-semibold">Administración de Proyectos</h2>
          <p className="mt-0.5 text-sm text-slate-500">Gestión completa de proyectos, horas y gastos.</p>
        </div>
        ...
        {(["projects", "hours", "expenses"] as const).map((tab) => (
          ...
          >
            {tab === "projects"
              ...
              : tab === "hours"
              ? "Control de horas"
              : `Gastos pendientes${pendingExpenses.length > 0 ? ` (${pendingExpenses.length})` : ""}`}
          </button>
        ))}
      </div>
      ...
      {activeTab === "projects" && (
        <div className="space-y-3">
          <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
            <label className="text-sm font-medium text-slate-600 whitespace-nowrap">Departamento</label>
            <select
          ...
          {loading ? (
            ...
            const filtered = filterDepartmentId
              ? projects.filter((p) => allUsers.find((u) => u.id === p.managerId)?.departmentId === filterDepartmentId)
              : projects;
            return (
            <Table headers={["Código", "Nombre", "Cliente", "Estado", "Período", "Modo horas", "Presup. h.", ""]}>
              {filtered.map((p) => (
                <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className="px-4 py-3 font-mono text-sm font-medium">{p.code}</td>
                  <td className="px-4 py-3">{p.name}</td>
                  <td className="px-4 py-3">{p.clientName ? <Badge>{p.clientName}</Badge> : <span className="text-slate-400">—</span>}</td>
                  ...
                  <td className="px-4 py-3">
                    <Badge>
                      {p.hourTrackingMode === "GENERAL"
                        ? "General"
                        : p.hourTrackingMode === "STRUCTURED"
                        ? "Especifica"
                        : p.hourTrackingMode === "BUILDING_BLOCK"
                        ? "Building Block"
                        : "Bolsa de horas"}
                    </Badge>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-500">
                  {filterDepartmentId ? "No hay proyectos en este departamento." : "No hay proyectos creados."}
                </td></tr>
              )}
            </Table>
            );
        ...
        <div className="space-y-4">
          <div className="flex gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex-1">
              ...
              <select className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={filterProjectId} onChange={(e) => setFilterProjectId(e.target.value)}>
            ...
            <div>
              ...
              <input type="month" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            <Table headers={["Fecha", "Trabajador", "Proyecto", "Tarea", "Horas", "Descripción"]}>
              {entries.map((e) => {
                const proj = projects.find((p) => p.id === e.projectId);
                const worker = allUsers.find((u) => u.id === e.userId);
                ...
                return (
                  <tr key={e.id} className={`border-t border-slate-100 ${isHigh ? "bg-amber-50" : "hover:bg-slate-50"}`}>
                    ...
                    <td className="max-w-xs truncate px-4 py-2 text-sm text-slate-600">{e.description}</td>
                  </tr>
                );
              })}
              {entries.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">Sin imputaciones.</td></tr>
              )}
            </Table>
          </div>
        </div>
      ...
      {activeTab === "expenses" && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <Table headers={["Proyecto", "Trabajador", "Partida", "Importe", "Descripción", ""]}>
            {pendingExpenses.map((e) => {
              ...
              return (
                <tr key={e.id} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button onClick={() => { setExpenseTarget(e); setRejectReason(""); setExpenseOpen(true); }}>Revisar</Button>
                  </td>
                </tr>
              );
            })}
            {pendingExpenses.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">No hay gastos pendientes.</td></tr>
            )}
          </Table>
        </div>
      )}
      ...
      <Modal open={formOpen} title={editProjectId ? "Editar proyecto" : "Nuevo proyecto"} onClose={() => setFormOpen(false)} panelClassName="max-w-5xl">
        <div className="space-y-3 pb-2">
          ...
          {!editProjectId && (
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-600">Responsable del proyecto</label>
              <select
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
              ...
              >
                ...
                {managers.map((u) => (
                  <option key={u.id} value={u.id}>{u.name} ({u.role === "admin" ? "Admin" : "Coordinador"})</option>
                ))}
              </select>
            </div>
          )}
          {!editProjectId && (form.hourTrackingMode === "STRUCTURED" || form.hourTrackingMode === "BUILDING_BLOCK") && (
            <div className="space-y-3 rounded-lg border border-slate-200 p-3">
              ...
              <p className="text-xs text-slate-500">
                Define paquetes y tareas con horas y/o fechas de inicio-fin para estructurar la imputación.
              </p>
              {form.hourTrackingMode === "BUILDING_BLOCK" && (
                ...
                <div key={wp.tempId} className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <div className="flex items-center justify-between">
                    ...
                    {form.hourTrackingMode === "STRUCTURED" && (
                      <button
                        ...
                        onClick={() => removeWorkPackageDraft(wp.tempId)}
                      >
                        Eliminar paquete
                      </button>
                    )}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Input
                      ...
                      value={wp.name}
                      onChange={(e) => updateWorkPackageDraft(wp.tempId, "name", e.target.value)}
                      disabled={form.hourTrackingMode === "BUILDING_BLOCK"}
                      placeholder="Ej: Desarrollo piloto"
                    />
                    <Input
                      label="Duración estimada (horas, opcional)"
                      type="number"
                      min="0"
                      value={wp.budgetHours}
                      onChange={(e) => updateWorkPackageDraft(wp.tempId, "budgetHours", e.target.value)}
                      placeholder="Ej: 120"
                    />
                    <Input
                      ...
                      type="date"
                      value={wp.startDate}
                      onChange={(e) => updateWorkPackageDraft(wp.tempId, "startDate", e.target.value)}
                    />
                    <Input
                      label="Fin (opcional)"
                      type="date"
                      value={wp.endDate}
                      onChange={(e) => updateWorkPackageDraft(wp.tempId, "endDate", e.target.value)}
                    />
                  </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {form.hourTrackingMode === "STRUCTURED" && (
                        <button
                          ...
                          onClick={() => addTaskDraft(wp.tempId)}
                        >
                          + Tarea
                        </button>
                      )}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {form.hourTrackingMode === "STRUCTURED" && (
                            <button
                              type="button"
                              className="text-xs font-medium text-red-600 hover:text-red-700"
                              onClick={() => removeTaskDraft(wp.tempId, task.tempId)}
                            >
                              Eliminar
                            </button>
                          )}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {form.hourTrackingMode === "STRUCTURED" && (
                            <Input
                              label="Nombre tarea"
                              value={task.name}
                              onChange={(e) => updateTaskDraft(wp.tempId, task.tempId, "name", e.target.value)}
                          ...
                          <Input
                            ...
                            onChange={(e) => updateTaskDraft(wp.tempId, task.tempId, "budgetHours", e.target.value)}
                            placeholder="Ej: 30"
                          />
                          <Input
                            label="Inicio (opcional)"
                            ...
                            onChange={(e) => updateTaskDraft(wp.tempId, task.tempId, "startDate", e.target.value)}
                          />
                          <Input
                            label="Fin (opcional)"
                            type="date"
                            value={task.endDate}
                            onChange={(e) => updateTaskDraft(wp.tempId, task.tempId, "endDate", e.target.value)}
                          />
                        </div>
                      </div>
                    ))}
          ...
          {!editProjectId && (
            <div className="space-y-3 rounded-lg border border-slate-200 p-3">
              <h4 className="text-sm font-semibold">Asignación de trabajadores</h4>
              <p className="text-xs text-slate-500">
                Departamento del responsable: <strong>{managerDepartmentName}</strong>.
              </p>
              {managerDepartmentId ? (
                <div className="grid grid-cols-2 gap-2">
                ...
                <p className="text-sm text-slate-500">Selecciona un responsable para cargar trabajadores del departamento.</p>
              )}
              {managerDepartmentId && candidateWorkers.length === 0 && (
                <p className="text-sm text-slate-500">No hay trabajadores disponibles en este departamento.</p>
              )}
            </div>
          )}
          {!editProjectId && (form.hourTrackingMode === "STRUCTURED" || form.hourTrackingMode === "BUILDING_BLOCK") && assignedUserIdsDraft.length > 0 && workPackagesDraft.length > 0 && (
            <div className="space-y-3 rounded-lg border border-slate-200 p-3">
              ...
              <p className="text-xs text-slate-500">
                Marca qué trabajadores podrán imputar en cada tarea.
              </p>
              {workPackagesDraft.map((wp) => (
                    ...
                    <div key={task.tempId} className="rounded border border-slate-200 bg-white p-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {assignedUserIdsDraft.map((userId) => {
                          const worker = allUsers.find((u) => u.id === userId);
                          if (!worker) return null;
                          const assignmentKey = getTaskAssignmentKey(wp.tempId, task);
                          const isAssigned = (taskAssignmentsDraft[assignmentKey] ?? []).includes(userId);
                          return (
                            <label key={`${task.tempId}-${userId}`} className="inline-flex items-center gap-2 rounded-md border border-slate-200 px-2 py-1 text-xs">
                              <input
                                type="checkbox"
                                checked={isAssigned}
                                onChange={() => toggleTaskAssignment(assignmentKey, userId)}
                                className="h-3.5 w-3.5 rounded border-slate-300 text-slate-900"
                              />
                              <span>{worker.name}</span>
                            </label>
        ...
        {selectedProject && (
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-3 gap-3 rounded-lg bg-slate-50 p-3">
              ...
              <div><span className="text-slate-500">Responsable</span><p>{selectedProject.managerName}</p></div>
            </div>
            <div>
              <h4 className="mb-2 font-semibold">Trabajadores asignados</h4>
              <div className="flex flex-wrap gap-2">
                {selectedProject.assignedUserIds.map((uid) => {
                  const worker = allUsers.find((u) => u.id === uid);
                  return <Badge key={uid}>{worker?.name ?? uid}</Badge>;
                })}
                {selectedProject.assignedUserIds.length === 0 && <span className="text-slate-400">Sin asignaciones.</span>}
              </div>
            </div>
            <div>
              <h4 className="mb-2 font-semibold">Paquetes de trabajo</h4>
              {selectedProject.workPackages.map((wp) => (
                <div key={wp.id} className="mb-3 rounded-lg border border-slate-200 p-3">
                  ...
                  {(wp.startDate || wp.endDate) && <p className="text-xs text-slate-400">{wp.startDate} → {wp.endDate}</p>}
                  <div className="mt-2 space-y-1">
                    {wp.tasks.map((t) => (
                      <div key={t.id} className="flex items-center justify-between rounded bg-slate-50 px-3 py-1">
                        <span className={t.isDefault ? "italic text-slate-500" : ""}>{t.isDefault ? "General (por defecto)" : t.name}</span>
              ...
              {selectedProject.workPackages.length === 0 && <p className="text-slate-400">Sin paquetes de trabajo definidos.</p>}
            </div>
          ...
          <div className="space-y-4 pb-2 text-sm">
            <div className="space-y-1 rounded-lg bg-slate-50 p-3">
              ...
              {expenseTarget.receiptUrl && (
                <a href={expenseTarget.receiptUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Ver ticket adjunto</a>
              )}
            </div>
            <div>
              <label className="mb-1 block text-slate-700">Motivo de rechazo (si rechazas)</label>
              <textarea className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" rows={2} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} />
            </div>
